...
def cmd_generate(args):
    ...
    world = generator.generate(
        config if config else prompt or "Generate Issavi hunt level 300"
    )
    t1 = time.time()
    ...
    print(
        f"  Tiles: {report.get('tiles', 0)}, Items: {report.get('items', 0)}, Spawns: {report.get('spawns', 0)}"
    )
    ...
    if result.get("png"):
        ...
        print("  Preview: generation skipped (no PIL or empty world)")
    ...
    print("    - generated.lua")
    print("    - generated.otbm")
    print("    - generated_preview.png")
    if report.get("files", {}).get("monster_xml"):
        print("    - generated.monster.xml")
    if report.get("files", {}).get("houses_xml"):
        print("    - generated.houses.xml")
    if report.get("files", {}).get("waypoints_xml"):
        print("    - generated.waypoints.xml")
...
def cmd_preview(args):
    ...
    if result.get("png"):
        ...
        print("  Preview: generation failed or PIL not installed")
        sys.exit(1)
...
def cmd_info(args):
    ...
    try:
        ...
        print("  [--] Not available at localhost:11434")
...
def _load_or_build_engine(args):
...
def cmd_knowledge_build(args):
    ...
    catalog_path = os.path.abspath(
        args.catalog or os.path.join(base_dir, "knowledge_catalog.json")
    )
    metrics_path = os.path.abspath(
        args.metrics or os.path.join(base_dir, "knowledge_metrics.json")
    )
    report_path = os.path.abspath(
        args.report or os.path.join(base_dir, "knowledge_report.md")
    )
...
def cmd_knowledge_search(args):
    ...
    for i, m in enumerate(result.matches[: args.k], 1):
        print(
            f"  {i}. {m.entry.name}  (score={m.score:.4f}, "
...
Examples:
  ...
  python cli.py info
    ...
    gen_parser = subparsers.add_parser(
        "generate", help="Generate a world from a prompt"
    )
    gen_parser.add_argument(
        "prompt", nargs="?", default="", help="Natural language prompt"
    )
    gen_parser.add_argument(
        "--type", choices=["hunt", "city", "dungeon"], help="Generator type"
    )
    ...
    exp_parser.add_argument(
        "--format", choices=["lua", "otbm"], default="lua", help="Export format"
    )
    exp_parser.add_argument(
        "--type", choices=["hunt", "city", "dungeon"], default="hunt"
    )
    ...
    val_parser.add_argument(
        "--input", "-i", required=True, help="OTBM file to validate"
    )
    ...
    # knowledge build
    kbuild = kp_subs.add_parser(
        "build", help="Build the knowledge dataset from sources"
    )
    kbuild.add_argument(
        "--sources", nargs="*", default=[], help="Source files (json/otbm)"
    )
    ...
    ksim.add_argument(
        "type", choices=["hunt", "city", "boss_room", "region", "quest", "raid"]
    )
    ...
    bp_build.add_argument(
        "--blueprints", nargs="*", default=[], help="Blueprint JSON files"
    )
    ...
    bp_evolve.add_argument(
        "--target", type=float, default=90.0, help="Target critic score"
    )
    bp_evolve.add_argument(
        "--generations", type=int, default=10, help="Max generations"
    )
    ...
    bp_fuse.add_argument(
        "--method", choices=["weighted", "interleave", "blend"], default="weighted"
    )
    ...
    bp_rec.add_argument(
        "--type", default="", help="Recommendation type (hunt, city, boss)"
    )
    bp_rec.add_argument(
        "--blueprints", nargs="*", default=[], help="Blueprint JSON files"
    )
    ...
    bp_gen.add_argument(
        "--blueprints", nargs="*", default=[], help="Reference blueprint files"
    )
    bp_gen.add_argument("--output", "-o", default=None)

    # autonomous subcommands
    auto = subparsers.add_parser(
        "autonomous", help="Autonomous World Designer commands"
    )
    auto_subs = auto.add_subparsers(
        dest="autonomous_command", help="Autonomous subcommand"
    )
    ...
    auto_opt.add_argument(
        "--max-iterations", type=int, default=20, help="Max optimization iterations"
    )
    auto_opt.add_argument("--output", "-o", default=None, help="Output directory")

    # autonomous benchmark
    auto_bench = auto_subs.add_parser(
        "benchmark", help="Run benchmark with multiple worlds"
    )
    auto_bench.add_argument(
...
def cmd_blueprint(args):
    ...
    if sub == "build":
        ...
        print(
            "  Usage: rme blueprint {build|similar|cluster|evolve|fuse|recommend|rank|generate}"
        )
...
def cmd_blueprint_evolve(args):
    ...
    result = engine.evolve(
        bp, target_critic_score=args.target, max_generations=args.generations
    )
    print(
        f"  Evolved {args.name}: gen {result.generation}, critic={result.critic_score:.1f}"
    )
...
def cmd_blueprint_rank(args):
    ...
    for i, r in enumerate(ranked, 1):
        print(
            f"  {i}. {r.blueprint_name:20s} rank={r.overall_rank:6.2f} critic={r.critic_score:5.1f}"
        )
...
def cmd_autonomous_optimize(args):
    ...
    print("[3/3] Optimization complete!")
    print(f"  Result ID: {result.result_id}")
    print(f"  Success: {result.success}")
    print(f"  Final Critic Score: {result.final_scores.get('critic', 0):.2f}")
    print(f"  Total Iterations: {len(result.iterations)}")
...
def cmd_autonomous_report(args):
    ...
    if report["history"]:
        ...
        for i, gen in enumerate(report["history"][-5:], 1):
            print(
                f"    {i}. {gen['prompt'][:50]}... (success={gen['success']}, "
                f"score={gen['final_scores'].get('critic', 0):.2f})"
            )
