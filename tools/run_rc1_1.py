...
def generate_audit(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    active_fallback_patterns = [
        re.compile(
            r"except\s+ImportError\s*:\s*[\r\n]+\s*(?:return\s+FakeEngine|return\s+simulated|return\s+fake|\S+\s*=\s*(?:FakeEngine|simulated|fake))",
            re.MULTILINE,
        ),
        re.compile(
            r"simulation_mode\s*=\s*True", re.IGNORECASE
        ),
        re.compile(
            r"mock_mode\s*=\s*True", re.IGNORECASE
        ),
        re.compile(
            r"fake_metrics\s*=\s*True", re.IGNORECASE
        ),
        re.compile(
            r"synthetic_results\s*=\s*True", re.IGNORECASE
        ),
    ]
