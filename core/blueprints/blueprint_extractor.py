...
class BlueprintExtractor:
    ...
    def _import_otbm(otbm_path: str) -> Optional[Dict[str, Any]]:
        """Import an OTBM only through the certified importer."""
        try:
            from core.otbm.otbm_importer import OTBMImporter

            import_result = OTBMImporter(validate=True).import_file(otbm_path)
            return import_result.get("world_dict") if import_result.get("success") else None
        except Exception:
            return None
