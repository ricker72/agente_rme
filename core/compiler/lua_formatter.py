...
class LuaFormatter:
    ...
    def _format_statement(self, statement: LuaStatement, lines: List[str], indent_level: int) -> None:
        ...
        if isinstance(statement, Assignment):
            ...
            lines.append(f"{prefix}if {self._format_expression(statement.condition)} then")
            ...
            step = f", {self._format_expression(statement.step)}" if statement.step is not None else ""
            lines.append(f"{prefix}for {statement.variable} = {self._format_expression(statement.start)}, {self._format_expression(statement.end)}{step} do")
            self._format_block(statement.body, lines, indent_level + 1)
            lines.append(f"{prefix}end")
        else:
            lines.append(f"{prefix}{self._format_expression(statement)}")
    ...
    def _format_expression(self, expression: LuaExpression) -> str:
        ...
        if isinstance(expression, FunctionCall):
            receiver = self._format_expression(expression.receiver) if expression.receiver is not None else ""
