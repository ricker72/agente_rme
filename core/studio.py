...
class AIMapStudio:
    ...
    def __init__(
        ...
        self.knowledge_graph = KnowledgeGraph(
            self.pattern_library, self.architecture_graph
        )
        self.planner = AIPlanner(
            self.pattern_library, self.architecture_graph, self.knowledge_graph
        )
    ...
    def _build_knowledge_base(
        self, item_docs: list[dict], monster_docs: list[dict], npc_docs: list[dict]
    ):
    ...
    def create_generation_prompt(
        self, description: str, monster_names: list[str], npc_names: list[str]
    ) -> str:
    ...
    def generate_script(
        ...
        if not world_plan_result.get("plan_valid", False):
            ...
            on_error(f"Invalid plan: {errors}")
            return
