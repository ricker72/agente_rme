...
class LuaGenerator:
    ...
    def generate(
        ...
        *,
        ...
        if world is not None and spawn_plan is None and self._looks_like_spawn_plan(world):
            spawn_plan = world
            world = None
        ...
        if spawn_plan is None and world is not None and not self._looks_like_spawn_plan(world):
            try:
    ...
    def _build(self, r: _ResolvedInputs, *, map_name: str) -> LuaScript:
        ...
        lines.append(f"-- Generated by RME Map AI Agent MVP V0.1")
        lines.append(
        ...
        lines.append(
            f"  -- Base position: ({r.base_x}, {r.base_y}, {r.base_z})"
        )
        lines.append("")
        ...
        for tile in r.tiles:
            ...
            ground_id = int(self._get_attr(tile, "ground", self._get_attr(tile, "ground_id", 0)) or 0)
            if ground_id > 0:
        ...
        if r.boss_spawn:
            ...
            lines.append(
                f'  b:setCreature("{b_name}", {b_interval}, Direction.SOUTH)'
            )
            creature_count += 1
            lines.append("")
