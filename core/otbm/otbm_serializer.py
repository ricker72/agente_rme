...
class OtbmSerializer:
    ...
    def serialize(self, world_model: Any) -> bytes:
        ...
        Args:
            world_model: WorldModel dataclass with tiles, spawns, cities, etc.

        Returns:
            bytes: OTBM binary ready to write to .otbm file.
        """
        ...
        if not all_x:
            # Empty map — create a minimal valid OTBM with one tile
            width = 1
            height = 1
            ...
            map_data = self.node.encode_map_data(
                description="Generated by OpenTibiaBR RME Agent",
                spawn_file="",
                house_file="",
            ...
            # Write TILE_AREA nodes per z-level
            for z in sorted(tiles_by_z.keys()):
                z_tiles = tiles_by_z[z]
                z_x = [self._get_x(t) for t in z_tiles]
                z_y = [self._get_y(t) for t in z_tiles]
                base_x = min(z_x)
                base_y = min(z_y)
                base_z = z
            ...
            if spawns:
                ...
                for spawn in spawns:
                    ...
                    interval = spawn.get("respawn", spawn.get("interval", spawn.get("spawntime", 60)))
                    # Also check tile-based spawns
                ...
                # Also check tile-based creature spawns
                for tile in tiles_list:
    ...
    def deserialize(self, data: bytes) -> Dict[str, Any]:
        """
        Deserialize OTBM binary back into a dictionary structure.

        Returns:
            dict with keys: version, width, height, item_version, tiles, spawns, towns, waypoints
        """
        ...
        # Root node does NOT have a size prefix in RME-style OTBM
        if offset >= len(data):
        ...
        # Root payload: version uint32, width uint16, height uint16, item_major uint32, item_minor uint32
        # Children extend to end of data
        end_root = len(data)
    ...
    def _deserialize_node(self, data: bytes, offset: int, result: dict) -> int:
        """Parse one node and append to result dict, returning new offset."""
        ...
        if node_type == OTBM_NODE_MAP_DATA:
            ...
            offset += 1
            while offset < end_node:
                offset = self._deserialize_tile(data, offset, result, base_x, base_y, base_z)
    ...
    def _deserialize_tile(self, data: bytes, offset: int, result: dict,
                          base_x: int, base_y: int, base_z: int) -> int:
        """Parse a TILE node."""
        node_type, size, offset = self.node.read_node_header(data, offset)
        ...
        # Check for TILE_FLAGS attribute
    ...
    def _deserialize_item(self, data: bytes, offset: int, items: list) -> int:
        ...
        while offset < end_item:
            ...
            if attr == ATTR_COUNT or attr == ATTR_SUBTYPE or attr == ATTR_CHARGES:
                item["count" if attr == ATTR_COUNT else (
                    "subtype" if attr == ATTR_SUBTYPE else "charges"
                )] = data[offset]
                offset += 1
                ...
                # Skip unknown attribute — read as uint8 value and move on
                break
    ...
    def _deserialize_monster(self, data: bytes, offset: int, spawn_entry: dict) -> int:
        """Parse a MONSTER node inside SPAWN_AREA."""
        node_type, size, offset = self.node.read_node_header(data, offset)
    ...
    # ------------------------------------------------------------------
    # Tile helpers (abstract over dataclass/dict tiles)
    # ------------------------------------------------------------------

    @staticmethod
    def _tile_to_dict(tile: Any) -> Dict[str, Any]:
        """Convert a tile object (dataclass or dict) to a plain dict."""
    ...
    def _get_spawn_name(spawn: Dict) -> str:
        """Extract monster name from a spawn dict."""
    ...
    # ------------------------------------------------------------------
    # HuntArea serialization (direct pipeline_runner integration)
    # ------------------------------------------------------------------

    def _get_attr(self, obj, name, default=None):
        """Get an attribute from either a dataclass object or a dict."""
    ...
    def serialize_hunt_area(self, hunt_area, spawn_plan=None) -> bytes:
        """
        Convert a HuntArea (+ optional SpawnPlan) directly into OTBM.

        This bridges the pipeline_runner output directly to .otbm files
        without needing a full WorldModel.

        Args:
            hunt_area: HuntArea dataclass or dict with tiles, rooms, base_x, base_y, base_z.
            spawn_plan: Optional SpawnPlan with spawn entries.

        Returns:
            bytes: OTBM binary data.
        """
        tiles_raw = self._get_attr(hunt_area, "tiles", {})
        if isinstance(tiles_raw, dict):
            # Could be dict of tuples or dict of string keys
            tiles_list = []
            for k, v in tiles_raw.items():
                if isinstance(v, dict):
                    # Try to build TileInfo-like dict
                    tiles_list.append(v)
                else:
                    tiles_list.append(v)
        else:
        ...
        for t in tiles_list:
            ...
            tz = self._get_attr(t, "z", base_z_val) if (isinstance(t, dict) and "z" in t) else getattr(t, "z", base_z_val)
            all_x.append(tx)
            all_y.append(ty)
            all_z.append(tz)
        ...
        # Spawns
        spawn_entries = []
        # From hunt_area.spawns: list of (x, y, monster, interval)
        for s in spawns_raw:
            if len(s) >= 3:
