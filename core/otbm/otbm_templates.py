...
class OtbmTemplateGenerator:
    ...
    def generate_monster_xml(self, world_model: Any) -> str:
        ...
        for monster in monsters:
            entries.append(
                f'  <monster name="{monster["name"]}" respawn="{monster.get("respawn", 60)}" />'
            )
        return "<monsters>\n" + "\n".join(entries) + "\n</monsters>\n"
    ...
    def generate_npc_xml(self, world_model: Any) -> str:
        ...
        for npc in npcs:
            entries.append(
                f'  <npc name="{npc["name"]}" x="{npc["x"]}" y="{npc["y"]}" z="{npc["z"]}" />'
            )
        return "<npcs>\n" + "\n".join(entries) + "\n</npcs>\n"
    ...
    def write_all(self, world_model: Any, base_path: Path) -> None:
        ...
        (base_dir / f"{base_name}.house.xml").write_text(
            self.generate_house_xml(world_model), encoding="utf-8"
        )
        (base_dir / f"{base_name}.monster.xml").write_text(
            self.generate_monster_xml(world_model), encoding="utf-8"
        )
        (base_dir / f"{base_name}.npc.xml").write_text(
            self.generate_npc_xml(world_model), encoding="utf-8"
        )
        (base_dir / f"{base_name}.zones.xml").write_text(
            self.generate_zones_xml(world_model), encoding="utf-8"
        )
    ...
    def _extract_monsters(self, world_model: Any) -> List[Dict[str, Any]]:
        ...
        for spawn in getattr(world_model, "spawns", []):
            if "monster" in spawn:
                monsters.append(
                    {"name": spawn["monster"], "respawn": spawn.get("respawn", 60)}
                )
        for boss in getattr(world_model, "bosses", []):
            monsters.append(
                {"name": boss.get("name", "boss"), "respawn": boss.get("respawn", 300)}
            )
        return monsters
    ...
    def _extract_npcs(self, world_model: Any) -> List[Dict[str, Any]]:
        ...
        for tile in getattr(world_model, "tiles", {}).values():
            if tile.creature and tile.creature.get("role") == "npc":
                npcs.append(
                    {
                        "name": tile.creature.get("name", "NPC"),
                        "x": tile.x,
                        "y": tile.y,
                        "z": tile.z,
                    }
    ...
    def _extract_zones(self, world_model: Any) -> List[Dict[str, int]]:
        ...
        for index, region in enumerate(
            getattr(world_model, "cities", [])
            + getattr(world_model, "dungeons", [])
            + getattr(world_model, "roads", []),
            start=1,
        ):
            x1 = region.get("x1", 0)
            y1 = region.get("y1", 0)
            ...
            zones.append(
                {
                    "name": region.get("name", f"zone{index}"),
                    "x1": x1,
                    "y1": y1,
                    "x2": x2,
                    "y2": y2,
