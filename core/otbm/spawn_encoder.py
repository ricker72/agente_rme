...
class SpawnEncoder:
    ...
    def encode_spawns_container(self, spawn_nodes: bytes) -> bytes:
        ...
        """
        return cast(bytes, self.node.encode_spawns(spawn_nodes))
    ...
    def _get_respawn(spawn: Any) -> int:
        ...
        if isinstance(spawn, dict):
            return int(cast(Any, spawn.get("respawn", spawn.get("interval", 60))))
    ...
    def _get_radius(spawn: Any) -> int:
        ...
        if isinstance(spawn, dict):
            return int(cast(Any, spawn.get("radius", spawn.get("spawn_radius", 3))))
