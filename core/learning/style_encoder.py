...
class StyleEncoder:
    ...
    KNOWN_STYLES = {
        ...
        "roshamuul": {
            ...
            "typical_walls": ["prison_wall", "iron_bars", "dark_brick", "stone_bars"],
            "lighting": "dark",
    ...
    def _extract_ground_distribution(
    ...
    def _extract_wall_distribution(self, region_data: Dict[str, Any]) -> Dict[str, int]:
        ...
        for tile in tiles:
            ...
            for item in items:
                if (
                    "wall" in item.get("type", "").lower()
                    or "wall" in item.get("name", "").lower()
                ):
    ...
    def _extract_corridor_statistics(
        self, region_data: Dict[str, Any]
    ) -> Dict[str, float]:
    ...
    def _build_style_profile(
        self, style: str, samples: List[Dict[str, Any]]
    ) -> StyleProfile:
        ...
        # Dominant lighting
        lighting = (
            Counter(lighting_votes).most_common(1)[0][0] if lighting_votes else "dim"
        )
        ...
        return StyleProfile(
            ...
            room_shape_distribution=shape_dist,
            connectivity_ratio=np.mean(connectivity_ratios)
            if connectivity_ratios
            else 0,
            lighting_preference=lighting,
            decoration_density=np.mean(decoration_densities)
            if decoration_densities
            else 0,
            common_effects=[],
    ...
    def save_profiles(self, output_path: str):
        ...
        data = {"version": "1.0", "styles": self.styles, "profiles": {}}
        ...
        if not profile:
            ...
            return {
                ...
                "description": style_info.get(
                    "description", f"Generated {style} style"
                ),
