...
class BlueprintRanker:
    ...
    def rank_blueprints(
        ...
        for bp in blueprints:
            ...
            scores["pattern_consistency"] = self._score_pattern_consistency(
                bp, reference_patterns
            )
    ...
    def _score_pattern_consistency(
        self, bp: Blueprint, reference_patterns: List[MinedPattern] = None
    ) -> float:
    ...
    def _compare_blueprint_to_pattern(
        self, bp: Blueprint, pattern: MinedPattern
    ) -> float:
    ...
    def _infer_layout_from_blueprint(self, bp: Blueprint) -> str:
        ...
        center_y = np.mean(ys)
        distances = [
            np.sqrt((x - center_x) ** 2 + (y - center_y) ** 2) for x, y in positions
        ]
    ...
    def _score_similarity_bonus(
        self, bp: Blueprint, similar_matches: List[SimilarityResult]
    ) -> float:
    ...
    def _generate_recommendations(
        self, bp: Blueprint, scores: Dict[str, float]
    ) -> List[str]:
        ...
        if scores.get("tile_quality", 0) < 0.5:
            if len(bp.tiles) < self.min_tile_count:
                recs.append(
                    f"Increase tile count (current: {len(bp.tiles)}, target: {self.min_tile_count}+)"
                )
