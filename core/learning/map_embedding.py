...
class MapEmbedder:
    ...
    def _generate_embedding_id(self, region_id: str, 
    ...
    def _extract_tile_features(self, region_data: Dict[str, Any]) -> np.ndarray:
        ...
        sorted_grounds = sorted(ground_types.items(), 
        ...
        features[24] = max(ground_types.values()) / total_tiles if total_tiles > 0 else 0
        features[25] = sum(1 for c in ground_types.values() if c == 1) / len(ground_types) if ground_types else 0
    ...
    def _extract_structure_features(self, region_data: Dict[str, Any]) -> np.ndarray:
        ...
        if rooms:
            ...
            features[5] = sum(1 for r in rooms if r.get("shape") == "rectangular") / len(rooms)
            features[6] = sum(1 for r in rooms if r.get("shape") == "irregular") / len(rooms)
    ...
    def _extract_style_features(self, region_data: Dict[str, Any]) -> np.ndarray:
        ...
        wall_types = region_data.get("wall_types", {})
        
        # Style encoding (one-hot-like, features 0-7)
        style_map = {
            "issavi": 0, "roshamuul": 1, "soulwar": 2, "library": 3,
            "falcon": 4, "cobra": 5, "yalahar": 6, "thais": 7
        }
    ...
    def similarity(self, embedding1: MapEmbedding, 
                  embedding2: MapEmbedding) -> float:
        """
    ...
    def save_embeddings(self, embeddings: List[MapEmbedding], 
                       output_path: str):
