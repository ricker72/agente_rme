...
class PatternEncoder:
    ...
    # Layout patterns
    LAYOUT_TYPES = ["grid", "linear", "radial", "organic", "clustered", 
                   "spiral", "branching", "hierarchical"]
    ...
    def _compute_radial_score(self, rooms: List[Dict[str, Any]]) -> float:
        ...
        angle_diffs = [sorted_angles[i+1] - sorted_angles[i] 
    ...
    def _compute_connectivity_metrics(self, connections: List[Dict[str, Any]], 
                                      rooms: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        # Average degree
        degrees = [len(adjacency.get(r.get("id", i), set())) for i, r in enumerate(rooms)]
        ...
        for i, room1 in enumerate(rooms[:10]):  # Limit for performance
            for j, room2 in enumerate(rooms[i+1:10]):
                ...
                path_length = self._bfs_shortest_path(adjacency, 
                                                     room1.get("id", ""), 
                                                     room2.get("id", ""))
                if path_length > 0:
    ...
    def _bfs_shortest_path(self, adjacency: Dict[str, Set[str]], 
    ...
    def _compute_architecture_metrics(self, region_data: Dict[str, Any]) -> Dict[str, float]:
    ...
    def _build_pattern_profile(self, pattern_type: str, 
                              samples: List[Dict[str, Any]]) -> PatternProfile:
        ...
        pattern_vector = [
            avg_grid, avg_linear, avg_radial, avg_symmetry, avg_axial,
            avg_degree, avg_clustering, avg_path,
            avg_tile_density, avg_feature_density, avg_empty_ratio, avg_wall_ratio,
            avg_complexity, avg_repetition, avg_hierarchy, avg_modularity
        ]
        ...
        dominant_layout = layout_types.most_common(1)[0][0] if layout_types else "organic"
    ...
    def compare_patterns(self, region1: Dict[str, Any], 
                        region2: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        numeric_features = [
            "symmetry_score", "axial_alignment", "avg_degree",
            "clustering_coefficient", "tile_density", "feature_density",
            "complexity_score", "modularity_score"
        ]
