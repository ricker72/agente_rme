...
class BlueprintCatalog:
    ...
    def by_size_range(
        self, min_area: int = 0, max_area: int = 100000
    ) -> List[Blueprint]:
    ...
    def by_tile_count_range(
        self, min_tiles: int = 0, max_tiles: int = 10000
    ) -> List[Blueprint]:
    ...
    def search(self, query: str) -> List[Blueprint]:
        ...
        for name, entry in self._index.items():
            if (
                ...
                or query_lower in entry.theme.lower()
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "largest_blueprint": (
                max(self._index.values(), key=lambda e: e.area).name
                if self._index
                else None
            ),
            "most_detailed": (
                max(self._index.values(), key=lambda e: e.tile_count).name
                if self._index
                else None
            ),
        }
    ...
    def get_most_used(self, count: int = 10) -> List[Blueprint]:
        ...
        sorted_entries = sorted(
            self._index.values(), key=lambda e: e.usage_count, reverse=True
        )
    ...
    def backup(self, backup_dir: str) -> str:
        ...
        backup_path = (
            Path(backup_dir)
            / f"blueprint_catalog_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
