...
class PatternMiner:
    ...
    def _mine_pattern_for_type(self, struct_type: str) -> Optional[MinedPattern]:
        ...
        for sample in samples:
            ...
            for spawn in sample.get("spawns", []):
                monster = (
                    spawn.get("monster", "") if isinstance(spawn, dict) else str(spawn)
                )
                if monster:
        ...
        if all_room_sizes:
            ...
            room_template = {
                ...
                "count_range": [
                    min(rooms_per_sample) if rooms_per_sample else 1,
        ...
        # Generation hints
        gen_hints = self._generate_hints(
            struct_type, layout_sig, required_grounds, required_items, spawn_patterns
        )
    ...
    def _compute_variability(self, samples: List[Dict[str, Any]]) -> float:
        ...
        for i in range(len(ground_vectors)):
            for j in range(i + 1, len(ground_vectors)):
                ...
                for k in keys:
                    diff += abs(
                        ground_vectors[i].get(k, 0) - ground_vectors[j].get(k, 0)
                    )
    ...
    def _generate_hints(
        ...
        hints = {
            "layout_preference": max(layout_sig, key=layout_sig.get)
            if layout_sig
            else "organic",
