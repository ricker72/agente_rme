...
def main():
    ...
    try:
        ...
        print(
            f"top_tiles:         {dict(sorted(analysis.tiles.items(), key=lambda x: x[1], reverse=True)[:5])}"
        )
        print(
            f"top_items:         {dict(sorted(analysis.items.items(), key=lambda x: x[1], reverse=True)[:5])}"
        )
