...
from core.knowledge import EntryType, KnowledgeEngine
...
class TestKnowledgeRanker(unittest.TestCase):
    ...
    def test_score_normalization(self):
        ...
        e1 = _entry("a", EntryType.HUNT)
        e1.quality_score = 20
        e2 = _entry("b", EntryType.HUNT)
        e2.quality_score = 50
        e3 = _entry("c", EntryType.HUNT)
        e3.quality_score = 80
        s1 = self.r.rank(e1)
        s2 = self.r.rank(e2)
        s3 = self.r.rank(e3)
...
class TestKnowledgeRecommender(unittest.TestCase):
    ...
    def test_recommend_by_text(self):
        result = self.rec.recommend_by_text(
            "circular hunt", k=3, entry_type=EntryType.HUNT
        )
        self.assertGreater(result.total, 0)
        for m in result.matches:
            self.assertEqual(m.entry.entry_type, EntryType.HUNT)
    ...
    def test_recommend_with_level_target(self):
        ...
        result = self.rec.recommend_for_entry(
            target,
            k=3,
            target_min_level=250,
            target_max_level=500,
        )
        self.assertGreater(result.total, 0)
