...
from core.playtest.combat_simulator import (
    CombatSimulator, MonsterStats, CombatResult, EncounterResult,
)
...
def dragon():
    return MonsterStats(
        name="Dragon", health=1000, attack=100,
        defense=60, magic_defense=50, experience=700,
        speed=80,
    )
...
def rat():
    return MonsterStats(
        name="Rat", health=20, attack=5,
        defense=2, magic_defense=2, experience=5,
        speed=80,
    )
...
def boss():
    return MonsterStats(
        name="Demon", health=8000, attack=400,
        defense=250, magic_defense=200, experience=5000,
        speed=80, is_boss=True,
    )
...
class TestCombatEncounter:
    def test_knight_vs_rat(self, sim):
        ...
        target = MonsterStats(
            name="Rat", health=20, attack=5,
            defense=2, magic_defense=2, experience=5, speed=80,
        )
    ...
    def test_knight_vs_dragon(self, sim):
        ...
        target = MonsterStats(
            name="Dragon", health=1000, attack=100,
            defense=60, magic_defense=50, experience=700, speed=80,
        )
        result = sim.simulate_encounter(player, target)
        assert result.damage_dealt > 0
        assert result.time_seconds > 0
    ...
    def test_druid_heals(self, sim):
        ...
        target = MonsterStats(
            name="Boss", health=5000, attack=300,
            defense=200, magic_defense=100, experience=3000, speed=80,
        )
        result = sim.simulate_encounter(player, target)
        assert result.damage_dealt > 0
        assert result.time_seconds > 0
    ...
    def test_sorcerer_spell_attack(self, sim):
        ...
        target = MonsterStats(
            name="Hydra", health=2100, attack=150,
            defense=80, magic_defense=70, experience=1500, speed=80,
        )
        result = sim.simulate_encounter(player, target)
        assert result.damage_dealt > 0
    ...
    def test_dps_positive(self, sim):
        player = sim.create_vocation_stats(Vocation.KNIGHT, 300)
        target = MonsterStats(
            name="Rotworm", health=65, attack=15,
            defense=8, magic_defense=5, experience=40, speed=80,
        )
        result = sim.simulate_encounter(player, target)
        assert result.dps > 0
        assert result.time_seconds > 0
...
class TestHuntRotation:
    ...
    def test_all_vocations_produce_results(self, sim):
        monsters = [
            MonsterStats(
                name="Goblin", health=80, attack=18,
                defense=10, magic_defense=8, experience=60, speed=80,
            )
        ]
...
class TestMonsterCreation:
    def test_create_monster(self):
        m = CombatSimulator.create_monster(
            name="Test", health=100, attack=10,
            defense=5, magic_defense=3, experience=20,
        )
        assert m.name == "Test"
        assert m.health == 100
    ...
    def test_create_boss(self):
        m = CombatSimulator.create_monster(
            name="Boss", health=10000, attack=500,
            defense=300, magic_defense=200, experience=10000,
            is_boss=True,
        )
